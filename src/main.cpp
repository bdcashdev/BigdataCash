...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (ActiveProtocol() >= REWARD_CHANGE) {
        ...
        if (nHeight >= 40000)   nSubsidy = 4 * COIN;
        if (nHeight >= 30000)   nSubsidy = 1 * COIN;
        if (nHeight >= 20001)   nSubsidy = 3 * COIN;
        if (nHeight >= 10000)   nSubsidy = 3 * COIN;
        if (nHeight >= 1)       nSubsidy = 2 * COIN;
        if (nHeight == 0)      nSubsidy = 1000000 * COIN;
    }
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
	...
	uiInterface.ThreadSafeMessageBox(_("You are on the wrong chain.  Please check debug.log for more information on resolving this.  Shutting down."), "", CClientUIInterface::MSG_ERROR);
        LogPrintf("Your block database is from a forked version of the correct chain.  Please visit https://github.com/BDCASHCoin/bootstrap to avoid any potential loss of future rewards and get on the proper chain.\n");
        StartShutdown();
        return false;
      }
    }
